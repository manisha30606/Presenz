...
export const updateStatus = async (req, res) => {
  try {
    ...
    try {
      const decoded = jwt.verify(token, process.env.JWT_SECRET);
      teacherIdFromToken = decoded.id || decoded._id || decoded.teaId;
    } catch (error) {
      return res.status(403).json({ message: "Invalid or expired token." });
    }
    ...
    if (!["ON", "OFF"].includes(status)) {
      return res.status(400).json({ message: "Invalid status value. Must be 'ON' or 'OFF'." });
    }
    ...
    // Match token ID with requested ID
    if (teacherIdFromToken !== teaId) {
      return res.status(403).json({ message: "You are not authorized to update this teacher's status." });
    }
    ...
    if (!teacher) {
      return res.status(404).json({ message: `Teacher with ID ${teaId} not found.` });
    }

    res.status(200).json({
      message: "Status updated successfully",
      status: teacher.teaStatus,
    });
  } catch (error) {
